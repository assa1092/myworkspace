...
public class Utils {
	...
	public static String makeDir(String uploadPath) {
		...
		for(String path : paths) {		// 2020 확인후 07 확인후 09 확인한다...
		...
		return paths[2];	// datePate 를 넘겨주어서 이 안에 업로드 파일을 넣는다.
	}
...
//		String datePate = monthPath + File.separator + arr[2];		// C://upload/2020/7/9
	...
	public static String makeThumbnail(String uploadPath, 
		...
		ImageIO.write(destImg, format, newFile);	// 썸네일이미지(destImg)를 format의 형태로 newFile이라는 이름으로 저장...
