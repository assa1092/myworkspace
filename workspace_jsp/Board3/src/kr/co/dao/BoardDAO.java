...
//		1번에 부모의 repRoot값, 2번에 부모의 repStep을 가져올것이다...
	...
	public PageTO page(int curPage) {
		...
		PageTO to = new PageTO(curPage);	// 페이징 처리를 위한 list
		List<BoardDTO> list = new ArrayList<BoardDTO>();
		...
		try {
			conn = dataFactory.getConnection();
			int amount = getAmount(conn);	// DB에서 총개수 amount 값 가져와서...
			to.setAmount(amount);			// 페이징 처리를 위한...to객체에 값을 알려준다...
			...
			to.setList(list);		// 페이징 처리를 위한 리스트...
	...
	private int getAmount(Connection conn) {
		...
		PreparedStatement pstmt = null;
		String sql = "select count(num) from board";	// 게시글의 개수 카운트한 값..보통 PK로 count한다...
		ResultSet rs = null;
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				amount = rs.getInt(1);	// 넘어올 값이 하나니까...1
			}
